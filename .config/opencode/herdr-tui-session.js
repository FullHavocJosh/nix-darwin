...
function setup(api) {
  ...
  function reconcileBlockers() {
    ...
    const members = new Set([
      selected,
      ...api.data.session.family(selected),
      ...blockers.values(),
    ]);
      ...
      for (const kind of ["permission", "form"]) {
        ...
        if (items === undefined) {
          for (const [key, owner] of blockers) {
            if (owner === member && key.startsWith(`${kind}:`))
              next.set(key, owner);
          }
          continue;
        }
    ...
    for (const [key, change] of blockerChanges) {
      if (
        hydrated.has(`${change.kind}:${change.id}`) &&
        next.has(key) === change.present
      ) {
  ...
  function syncSelection() {
    ...
    nextSelectionAt =
      delay === undefined ? Number.POSITIVE_INFINITY : Date.now() + delay;
  }
  ...
  function receive({ details: event }) {
    ...
    if (event.type === "session.deleted") {
      const affected =
        data.sessionID === selected ||
        [...blockers.values()].includes(data.sessionID);
      ...
      for (const [key, owner] of blockers)
        if (owner === data.sessionID) blockers.delete(key);
      for (const [key, change] of blockerChanges) {
    ...
    syncSelection();
    const id =
      event.type === "form.created" ? data.form.sessionID : data.sessionID;
